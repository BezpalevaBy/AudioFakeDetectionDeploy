...
class aasist3(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.w2v_encoder = Wav2Vec2Encoder(
            cache_dir=w2v_cache_dir, load_pretrained=load_pretrained
        )
        ...
        self.GAT_layer_S = GraphAttentionLayer(
            filts[-1][-1], gat_dims[0], temperature=temperatures[0], size=size
        )
        self.GAT_layer_T = GraphAttentionLayer(
            filts[-1][-1], gat_dims[0], temperature=temperatures[1], size=size
        )
